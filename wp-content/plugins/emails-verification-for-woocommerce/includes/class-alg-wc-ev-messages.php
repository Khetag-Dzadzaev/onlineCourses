...
if (! class_exists('Alg_WC_Email_Verification_Messages')) :
	...
	class Alg_WC_Email_Verification_Messages {
		...
		function get_error_message($user_id) {
			$notice       = do_shortcode(get_option(
		...
		function get_resend_message($code = '1') {
			...
			$success_message = do_shortcode(get_option(
			...
			$resend_messages = apply_filters(
				'alg_wc_ev_resend_status_codes',
				array(
					'1' => array(
						'type'  => 'success',
						'msg'   => $success_message,
		...
		function get_success_message() {
			return do_shortcode(get_option(
				'alg_wc_ev_success_message',
		...
		function get_already_verified_message($user_id) {
			$notice = do_shortcode(get_option(
				'alg_wc_ev_already_verified_message',

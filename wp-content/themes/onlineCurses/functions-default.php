...
function pagenavi_pagination($html)
{
	...
	$out = str_replace('<span aria-current=\'page\' class=\'current\'', '<li class="page-item active"><span class="page-link"', $out);
	$out = str_replace('<span class=\'extend\'', '<li class="page-item"><span class="page-link"', $out);
...
function symocoPay($order)
{
	$access_token = (new WC_Symoco_Gateway())->get_option('access_token');
	...
	$response = wp_remote_post('https://api.symoco.com/v1/orders', $args);

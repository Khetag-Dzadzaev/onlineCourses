...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($_POST['post_place'])) {
		$errors['post_place'] = 'Введите содержимое поста';
	}
	if (empty($_POST['post_place_en'])) {
		$errors['post_place_en'] = 'Введите содержимое поста';
	}
	...
	foreach ($langArr as $el) {
		...
		if ($el == 'ru') {
			...
			$new_post['meta_input']['product_place'] = $_POST['post_place'];
		}
		if ($el == 'en') {
			$new_post['post_title'] = sanitize_text_field($_POST['post_title_en']);
			$new_post['post_content'] = $_POST['post_article_en'];
			$new_post['meta_input']['product_place'] = $_POST['post_place_en'];
		}
